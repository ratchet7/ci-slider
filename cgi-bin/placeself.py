...
userbrowser=environ['HTTP_USER_AGENT']
timestamp=PubGlobals.timestamp()
randomnum=random.random()
randomstr=str(randomnum)
user_id=timestamp+randomstr

# Assigns to treatment group

M=random.sample(['dme', 'dfe', 'dmw', 'dfw', 'rme', 'rfe', 'rmw', 'rfw'],1) 

partygenderquote=''.join(M)

currentq=1

#PubGlobals.firstQuery(ipaddress, userbrowser, timestamp, user_id, currentq, partygenderquote)
...
<p>Let's get started.  Where would you place <b>yourself</b> on the slider?
<br/>
<br/>

<p> <b>First</b>, indicate your <b>best guess</b> by dragging the arrowhead labeled "You" to to a location on the slider. 

<p> <b>Second</b>, use the red circle to adjust the <b>answer range</b> so that you're
95 percent certain that your true ideology is in the answer range.
<br/>
<br/>


<p> Click Next when you're done.


<div id="box" class="jxgbox" style="width:750px; height:225px;"></div>

<form method="post" name="myform" action="dems8.py" onsubmit="sendSliderToForm(selfplaceslider.Value(), 'placementmean'); sendSliderToForm(varianceslider.Value(), 'placementvar')"; >
...
<script type="text/javascript">
 ...
 ilinerighttxt2 = b.create('text',[-28,-3.5, 'Liberal'], {fixed:true, fontSize:12});
  ...
  var selfplacelabel = b.create('text', [function(x){ return selfplaceslider.Value()-1.6;}, 4,'You'],
   {fontSize:14});
